...
{
    Contacts.find(function(err, editContactCollection)
    {
      // Database error
      if(err)
      {
      ...
      res.render('index', { title: 'Update Contact', page: 'update-contact', contacts: editContactCollection, displayName:  UserDisplayName(req)  });
    });
}
